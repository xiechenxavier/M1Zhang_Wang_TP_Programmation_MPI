...
void DistributedDP(vector<int>& weights, vector<int>& values, int knapsackBound,
  ...
  for (int j = 0; j < nb_nums; j++) {
    if (rankID != nbprocs - 1)
      MPI_Bsend(&local_matrix[ceil_charge_unit-1-j], 1, MPI_INT, rankID + 1, 1, MPI_COMM_WORLD);
  }
  for (int i = 1; i < nbItems; i++) {
    ...
    if (rankID != nbprocs - 1) {
      for (int k = 0; k < nb_nums; k++) { 
        MPI_Bsend(&local_matrix[ceil_charge_unit - k - 1], 1, MPI_INT, rankID + 1, 1, MPI_COMM_WORLD);
      }
    }
